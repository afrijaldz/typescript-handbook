...
Using [mapping modifiers](/mapped-types.html#mapping-modifiers), you can remove `readonly` attributes.
...
This also means that we can avoid overloads entirely by instead using [generic functions](/functions.html#generic-functions).
...
Tuples types can be used in [rest parameters and arguments](/functions.html#rest-parameters-and-arguments), so that the following:
